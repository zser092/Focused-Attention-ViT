"""
Vision Transformer (ViT) with Multi-Head Latent Attention (MHLA) Experiment
with Pre-trained Weights

This module implements a training pipeline for the Vision Transformer model
with Multi-Head Latent Attention (MHLA) using pre-trained weights. It calculates
and records theoretical space/time complexity and actual resource usage.
"""
...
class PretrainedMHLAViTExperiment:
    """
    Experiment class for Vision Transformer with MHLA training using pre-trained weights.
    """
    ...
    def setup(self):
        ...
        # Create model with MHLA
        print("Creating Vision Transformer model with MHLA...")
        self.model = VisionTransformerMHLA(
            img_size=self.img_size,
        ...
        print("Loading pre-trained weights into model...")
        ...
        # Now map weights from temp_model to MHLA model
        ...
        if self.freeze_layers:
            if isinstance(self.freeze_layers, bool) and self.freeze_layers:
                # Freeze all layers except the head and latent projections
                for name, param in self.model.named_parameters():
                    if 'head' not in name and 'latent_proj' not in name:
        ...
        # For MHLA, we modify the complexity calculation to account for window-based attention
        ...
        # Adjust complexity for MHLA
        # The main difference is in the attention mechanism
        ...
        # Calculate complexity ratio between standard attention and MHLA
        # Standard attention: O(N²)
        # MHLA: O(N*W)
        complexity_ratio = self.window_size / num_tokens
        
        # Calculate MHLA complexity
        mhla_complexity = {
            'parameters': traditional_complexity['parameters'] + self.depth * self.embed_dim,  # Additional latent projection parameters
            'flops': traditional_complexity['flops'] * complexity_ratio,
            'time_complexity': traditional_complexity['time_complexity'] * complexity_ratio,
            'space_complexity_bytes': traditional_complexity['space_complexity_bytes'],
            'space_complexity_mb': traditional_complexity['space_complexity_bytes'] / (1024 * 1024)
        }
        
        self.metrics['theoretical'] = mhla_complexity
    ...
    def train(self):
        ...
        if self.freeze_layers:
            # If layers are frozen, only optimize the head and latent projections
            params = []
            for name, param in self.model.named_parameters():
            ...
            # Use different learning rates for pre-trained layers, latent projections, and head
            params = [
                {'params': [p for n, p in self.model.named_parameters() 
                           if 'head' not in n and 'latent_proj' not in n], 'lr': self.learning_rate},
                {'params': [p for n, p in self.model.named_parameters() 
                           if 'latent_proj' in n], 'lr': self.learning_rate * 5},  # Higher LR for latent projections
                {'params': self.model.head.parameters(), 'lr': self.head_learning_rate}
            ]
            optimizer = optim.AdamW(params, weight_decay=self.weight_decay)
    ...
    def save_results(self):
        ...
        results = {
            ...
            'num_heads': self.num_heads,
            'window_size': self.window_size,
            'complexity_reduction_ratio': self.metrics['complexity_reduction']['ratio'],
        ...
        # Save to CSV
        csv_path = os.path.join(self.results_dir, 'exp4_pretrained_mhla.csv')
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='Pretrained ViT + MHLA Experiment')
